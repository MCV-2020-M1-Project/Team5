...
def precompute_MQ(listMImgs, k, nBins, typeHist):

    for j in range(len(listMImgs)):
        print("precalculating BBDD image: " + str(j))
...
# Precalculate Museum
Mpicklepath = "subMHist.pkl"
if os.path.isfile(Mpicklepath):
    print("Importing hists from pickle")
    infile = open(Mpicklepath, 'rb')
    listMHists = pickle.load(infile)
    infile.close()
else:
    listMHists = precompute_MQ(listMImgs, k, nBins, typeHist)
    outfile = open(Mpicklepath, 'wb')
    pickle.dump(listMHists, outfile)
    outfile.close()
